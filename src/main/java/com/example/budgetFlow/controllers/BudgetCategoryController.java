...
public class BudgetCategoryController {
    ...
    public ResponseEntity<BudgetCategory> create(@RequestBody @Valid BudgetCategoryDTO dto) {
        ...
        Budget budget = requireOwnedBudget(dto.getBudgetId());
        Category category = requireOwnedCategory(dto.getCategoryId());
    ...
    public ResponseEntity<BudgetCategory> getById(@Parameter(description = "BudgetCategory ID") @PathVariable Long id) {
        ...
        userService.assertOwnership(bc.getBudget().getUser().getId());
    ...
    public ResponseEntity<List<BudgetCategory>> getByBudget(@Parameter(description = "Budget ID") @PathVariable Long budgetId) {
        requireOwnedBudget(budgetId);
        return ResponseEntity.ok(budgetCategoryService.getByBudgetId(budgetId));
    }
    ...
    public ResponseEntity<BudgetCategory> update(@Parameter(description = "BudgetCategory ID") @PathVariable Long id,
        ...
        userService.assertOwnership(existing.getBudget().getUser().getId());

        existing.setBudget(requireOwnedBudget(dto.getBudgetId()));
        existing.setCategory(requireOwnedCategory(dto.getCategoryId()));
    ...
    private Budget requireOwnedBudget(Long budgetId) {
        Budget budget = budgetService.getBudgetById(budgetId);
        userService.assertOwnership(budget.getUser().getId());
        return budget;
    }

    private Category requireOwnedCategory(Long categoryId) {
