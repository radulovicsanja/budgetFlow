      ...
      <div>
        ...
        >
          ...
          <text
              ...
              y={baseY - hExpenses - 8}
              textAnchor="middle"
              className="chart-legend expenses"
          >
            {expenses.toFixed(0)} €
          </text>
        </svg>
...
export function LineChart({
                ...
                {data.map((d, i) => {
                  ...
                  return (
                      <text
                          ...
                          className={
                            active
                                ? 'chart-label chart-label-active'
                                : 'chart-label'
                          }
                      >
                        {d.label.slice(5)}
  ...
  const y = (value: number): number =>
      h -
      padB -
      (value / max) * (h - padT - padB)

  const line = (key: 'income' | 'expenses'): string =>
      data
          .map(
              (d, i) =>
                  `${i === 0 ? 'M' : 'L'} ${x(i)} ${y(d[key])}`,
  ...
  return (
      <svg
          ...
          className="chart-svg"
          role="img"
          aria-label="Trend prihoda i rashoda"
      >
        {ticks.map((t) => {
          const val = max * t
          const yy = y(val)
  ...
  return (
      <div className="pie-wrap">
        <svg
            ...
            className="chart-svg pie"
            role="img"
            aria-label="Potrošnja po kategorijama"
        >
